...
# def print_dieren(tak):
#     is_dier = not isinstance(tak,dict)
#     print = is_dier
...
def eindig_spel(blad, stam, richting):
    if vraag_ja_nee('Is je dier misschien een ' + blad + '?'):
        print('Yes! Ik heb het geraden! Ik ben zo goed!')
    else:
        bewaar_nieuw_dier(stam, welke_kant(richting), blad)

def bewaar_nieuw_dier(hogere_tak, kant, oud_dier):
    nieuw_dier = input('Oh, wat jammer dat ik het niet heb geraden! Welk dier zat je aan te denken? ').lower()
    nieuw_dier = nieuw_dier.translate(str.maketrans('', '', string.punctuation)) # haal leestekens eruit
    
    if nieuw_dier.startswith('een '):
        nieuw_dier = nieuw_dier[4:len(nieuw_dier)]

# Check met de speler of de vraag klopt
    while True:
        ...
        if vraag_ja_nee("De nieuwe vraag is:",nieuwe_vraag,"Klopt deze vraag om onderscheid te maken tussen een " + oud_dier.lower() + ' en een ' + nieuw_dier.lower() + '? '):
            break
